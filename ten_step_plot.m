...
for k = (1:10) % 10-step prediction using 30-length original data
    Y = textread('E:\RDE\code\traindata_08_04_50.txt'); %Y is a 24 Dimension EEG signal
    xx = Y(1:end,:)';  
    trainlength = 50+k-1;
    traindata = xx(:,1:trainlength);
    dlmwrite('E:\RDE\code\traindata_08_04_50.txt',traindata,'delimiter','\t','newline','pc');

    D = size(traindata,1); %D = 24

    s = 3000;% the number of non-delay embedding
