...
%EEG signal Normal
Y_origin=importdata('F:\DFA\DFA_DATA\RDE所用数据\癫痫\20121101111605.txt'); 
xx_origin = Y_origin(2000+1:2000+30,:)';  
traindata = xx_origin;
realdata = Y_origin(2000+1:2000+35,:)'; %5-step real data value
dlmwrite('F:\DFA\DFA_DATA\RDE所用数据\癫痫\traindata_605.txt',traindata,'delimiter','\t','newline','pc');
dlmwrite('F:\DFA\DFA_DATA\RDE所用数据\癫痫\realdata_605.txt',realdata,'delimiter','\t','newline','pc');
...
for k = (1:5) % 10-step prediction using 30-length original data
    ...
    Y = textread('F:\DFA\DFA_DATA\RDE所用数据\癫痫\traindata_605.txt'); %Y is a 24 Dimension EEG signal
    traindata = Y(1:end,:);  
    ...
    dlmwrite('F:\DFA\DFA_DATA\RDE所用数据\癫痫\prevalue_605.txt',pre_value,'delimiter','\t','newline','pc');
end
