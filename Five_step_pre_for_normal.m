...
%EEG signal Normal
Y_origin=importdata('F:\DFA\DFA_DATA\RDE所用数据\正常\20121031103719.txt'); 
xx_origin = Y_origin(2000+1:2000+30,:)';  
traindata = xx_origin;
realdata = Y_origin(2000+1:2000+35,:)'; %5-step real data value
dlmwrite('F:\DFA\DFA_DATA\RDE所用数据\正常\traindata_719.txt',traindata,'delimiter','\t','newline','pc');
dlmwrite('F:\DFA\DFA_DATA\RDE所用数据\正常\realdata_719.txt',realdata,'delimiter','\t','newline','pc');
...
for k = (1:5) % 10-step prediction using 30-length original data
    ...
    Y = textread('F:\DFA\DFA_DATA\RDE所用数据\正常\traindata_719.txt'); %Y is a 24 Dimension EEG signal
    traindata = Y(1:end,:);  
    ...
    dlmwrite('F:\DFA\DFA_DATA\RDE所用数据\正常\prevalue_719.txt',pre_value,'delimiter','\t','newline','pc');
end
